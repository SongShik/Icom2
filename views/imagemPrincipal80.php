<div class="slider-principal-80">
    <div id="slider-imagens-80" class="owl-carousel">

    <div class="item">
            <!--<div style="background-image: url(img/coracao.jpg);" class=""></div>-->
            <div style="background-image: url(img/coracaoW.jpg);" class="d-none d-md-block"></div>
            <div style="background-image: url(img/coracaoM.jpg);" class="d-md-none"></div>
    </div>


    </div>

</div>
<section class="section-cotar">
<div class="text-center cotar font-weight-bold">Veja as vantagens</div>
<!--
<form class="form-inline justify-content-center cotacao"
action="http://13.82.100.46/icomcotacao/save-email">
<div class="row row-cotacao">
    <div class="col-10 ">
        <input type="email" class="email " required
            placeholder="Insira seu email para começar a cotação">
    </div>

    <div class="col-2 ">
        <button type="submit" value="Submit" class="botao-cotacao ">
            <svg version="1.1" id="Camada_1" xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 32 32"
                style="enable-background:new 0 0 32 32;" xml:space="preserve" class="svg-buscar">

                <g id="Page-1">
                    <g id="icon-111-search">
                        <path id="search"  d="M20.06,23.04c-2.08,1.48-4.62,2.35-7.36,2.35C5.68,25.39,0,19.71,0,12.7S5.68,0,12.7,0
s12.7,5.68,12.7,12.7c0,2.74-0.87,5.28-2.35,7.36l8.34,8.34c0.82,0.82,0.82,2.14,0,2.95l-0.04,0.04c-0.81,0.81-2.14,0.82-2.95,0
L20.06,23.04L20.06,23.04z M12.7,22.41c5.36,0,9.71-4.35,9.71-9.71s-4.35-9.71-9.71-9.71S2.99,7.33,2.99,12.7
S7.33,22.41,12.7,22.41L12.7,22.41z" />
                    </g>
                </g>
            </svg></button>

    </div>
</div>

</form>
-->
<div class="separador"></div>
</section>


